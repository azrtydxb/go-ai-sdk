...
func TestConformance(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "huggingface")
	defer srv.Close()
...
func TestAuthHeaderAndModelSent(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "huggingface")
	defer srv.Close()
	m := New(WithAPIKey("secret-k"), WithBaseURL(srv.URL)).Model("meta-llama/Meta-Llama-3-8B-Instruct:together")
	...
	if err := json.Unmarshal(reqs[0], &body); err != nil || body.Model != "meta-llama/Meta-Llama-3-8B-Instruct:together" {
