...
func TestConformance(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "qwen")
	defer srv.Close()
...
func TestAuthHeaderAndModelSent(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "qwen")
	defer srv.Close()
	m := New(WithAPIKey("secret-k"), WithBaseURL(srv.URL)).Model("qwen-max")
	...
	if err := json.Unmarshal(reqs[0], &body); err != nil || body.Model != "qwen-max" {
...
func TestMaxTokensUsesMaxTokensField(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "qwen")
	defer srv.Close()
	m := New(WithAPIKey("k"), WithBaseURL(srv.URL)).Model("qwen-max")
...
func TestEmbeddingRequestShape(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "qwen")
	defer srv.Close()
	em := New(WithAPIKey("k"), WithBaseURL(srv.URL)).EmbeddingModel("text-embedding-v3")
	if em.MaxBatchSize() != 10 {
	...
	if body.Model != "text-embedding-v3" {
