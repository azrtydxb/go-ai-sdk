...
func newRerankFixtureServer(t *testing.T, capture *capturedRerankRequest) *httptest.Server {
	...
	mux.HandleFunc("/rerank", func(w http.ResponseWriter, r *http.Request) {
		...
		resp := rerankResponse{
			...
			Usage: rerankUsageWire{TotalTokens: 12},
		}
...
func TestRerankRequestShape(t *testing.T) {
	...
	model := New(WithAPIKey("k"), WithBaseURL(srv.URL)).RerankingModel("rerank-2")
	...
	if captured.Model != "rerank-2" {
		t.Errorf("model = %q, want rerank-2", captured.Model)
	}
	...
	if _, ok := captured.raw["documents"]; !ok {
		t.Error(`documents field missing from request body, want present (voyage uses "documents", not "input")`)
	}
	if _, ok := captured.raw["input"]; ok {
		t.Error(`input field present in request body, want absent (voyage rerank uses "documents", not "input")`)
	}
...
func TestRerankResultOrderPreservedAndUsage(t *testing.T) {
	...
	if resp.Usage.TotalTokens != 12 {
		t.Errorf("Usage.TotalTokens = %d, want 12 (from usage.total_tokens)", resp.Usage.TotalTokens)
	}
	if len(resp.Raw) == 0 {
...
func TestRerankModelMetadata(t *testing.T) {
	...
	if got := model.ModelID(); got != "rerank-2" {
		t.Errorf("ModelID() = %q, want rerank-2", got)
	}
...
// TestRerankShapeDiffersFromMixedbread proves the field-name difference the
// task brief calls out: a request captured from voyage's rerankRequest MUST
// use "documents"/"top_k" with "relevance_score" in the response, a shape
// that would fail to decode Mixedbread's "input"/"score" wire format
// (see providers/mixedbread/rerank_test.go for the converse assertion).
func TestRerankShapeDiffersFromMixedbread(t *testing.T) {
	var captured capturedRerankRequest
	srv := newRerankFixtureServer(t, &captured)
	model := New(WithAPIKey("k"), WithBaseURL(srv.URL)).RerankingModel("rerank-2")
	...
	// Mixedbread's request has no "documents" key at all (it uses
	// "input"); Voyage's must have "documents" for this test to be
	// meaningful proof of the difference.
	if _, ok := captured.raw["documents"]; !ok {
		t.Fatal(`voyage request missing "documents" key`)
	}
	if _, ok := captured.raw["return_input"]; ok {
		t.Error(`voyage request has "return_input" key, want absent (that's a Mixedbread-only field)`)
	}
}
