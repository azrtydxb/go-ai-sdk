// Package minimax provides the MiniMax provider: MiniMax's API is
...
// Provider is a MiniMax-backed provider.LanguageModel factory.
...
func New(opts ...Option) *Provider {
	p := &Provider{apiKey: os.Getenv("MINIMAX_API_KEY"), baseURL: defaultBaseURL, httpClient: http.DefaultClient}
	for _, o := range opts {
		o(p)
	}
	return p
...
// Model returns a provider.LanguageModel for the given MiniMax model ID.
func (p *Provider) Model(id string) provider.LanguageModel {
	return openaicompat.NewLanguageModel(openaicompat.Config{
		...
		NativeJSON: true,
		// MiniMax documents "max_tokens", not OpenAI's current
