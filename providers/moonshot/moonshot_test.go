...
func TestConformance(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "moonshot")
	defer srv.Close()
...
func TestAuthHeaderAndModelSent(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "moonshot")
	defer srv.Close()
	m := New(WithAPIKey("secret-k"), WithBaseURL(srv.URL)).Model("moonshot-v1-8k")
	...
	if err := json.Unmarshal(reqs[0], &body); err != nil || body.Model != "moonshot-v1-8k" {
...
// TestResponseFormatJSONObjectOnly asserts that Moonshot's preset always
// serializes response_format as {"type":"json_object"}, dropping any Schema
// — Moonshot rejects json_schema response_format.
