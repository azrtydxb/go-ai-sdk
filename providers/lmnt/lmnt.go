...
// wireError matches LMNT's error body: {"error":"..."}.
type wireError struct {
	Error string `json:"error"`
}

// errorMessage tries to parse LMNT's error body shape {"error":"..."}.
...
// applyProviderOptions merges providerOptions["lmnt"] (when it is a
...
func applyProviderOptions(reqBytes []byte, providerOptions map[string]any) ([]byte, error) {
	opts, _ := providerOptions["lmnt"].(map[string]any)
	if len(opts) == 0 {
		return reqBytes, nil
	}
