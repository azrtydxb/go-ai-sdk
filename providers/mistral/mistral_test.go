...
// representation for Mistral: setting it produces no new top-level keys in
// the request.
func TestRequestShapeReasoningIgnored(t *testing.T) {
	...
	for _, key := range []string{"reasoning", "thinking", "reasoning_effort", "budget_tokens"} {
		if _, ok := raw[key]; ok {
			t.Errorf("request unexpectedly contains %q (Mistral has no reasoning knob): %s", key, fs.rawBody())
		}
	}
}
