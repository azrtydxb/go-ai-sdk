...
func TestStreamTranscribe_HandshakeAndQuery(t *testing.T) {
	...
	go func() {
		...
		_ = br
		_ = websockettest.WriteMessage(conn, websockettest.OpText, []byte(`{"type":"Metadata"}`))
	}()
...
func TestStreamTranscribe_HandshakeHeaders(t *testing.T) {
	...
	if gotAuth != "Token dg-key" {
		t.Errorf("Authorization = %q, want %q (Headers must not clobber auth)", gotAuth, "Token dg-key")
	}
}
...
// clean non-socket termination (a "Metadata" message), not just leave the
// TCP connection lingering until the caller eventually calls Close(). ---

func TestStreamTranscribe_MetadataEndClosesUnderlyingConn(t *testing.T) {
	l, baseURL := listenerBaseURL(t)
	...
	go func() {
		...
		// socket as soon as readLoop saw the clean Metadata end, rather
		// than leaving the TCP connection open until some later Close().
	...
	case ok := <-tornDown:
		if !ok {
			t.Error("expected the client to have closed its socket after the clean Metadata end without stream.Close() being called")
		}
...
func TestStreamTranscribe_SendAfterCloseSend(t *testing.T) {
	...
	go func() {
		...
		defer func() { _ = conn.Close() }()
		_ = websockettest.Upgrade(conn)
		_, _, _ = websockettest.ReadMessage(conn) // CloseStream
		_ = websockettest.WriteClose(conn, 1000, "")
	}()
