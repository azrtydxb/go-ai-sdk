...
func TestConformance(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "deepinfra")
	defer srv.Close()
...
func TestAuthHeaderAndModelSent(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "deepinfra")
	defer srv.Close()
	m := New(WithAPIKey("secret-k"), WithBaseURL(srv.URL)).Model("meta-llama/Meta-Llama-3-8B")
	...
	if err := json.Unmarshal(reqs[0], &body); err != nil || body.Model != "meta-llama/Meta-Llama-3-8B" {
...
func TestEmbeddingRequestShape(t *testing.T) {
	srv := compattest.NewFixtureServer(t, "deepinfra")
	defer srv.Close()
	em := New(WithAPIKey("k"), WithBaseURL(srv.URL)).EmbeddingModel("BAAI/bge-large-en-v1.5")
	if em.MaxBatchSize() != 1024 {
	...
	if body.Model != "BAAI/bge-large-en-v1.5" {
