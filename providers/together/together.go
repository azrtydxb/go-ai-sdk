...
// Provider is a Together-backed provider.LanguageModel / EmbeddingModel
// factory.
...
// New creates a new Together Provider.
func New(opts ...Option) *Provider {
	p := &Provider{apiKey: os.Getenv("TOGETHER_AI_API_KEY"), baseURL: defaultBaseURL, httpClient: http.DefaultClient}
	for _, o := range opts {
		o(p)
	}
	return p
...
// Model returns a provider.LanguageModel for the given Together model ID.
func (p *Provider) Model(id string) provider.LanguageModel {
	return openaicompat.NewLanguageModel(openaicompat.Config{
		...
		NativeJSON: true,
		// Together documents "max_tokens", not OpenAI's current
		// "max_completion_tokens" — the latter is silently dropped.
		MaxTokensParam: "max_tokens",
	}, id)
}

// EmbeddingModel returns a provider.EmbeddingModel for the given Together
// embedding model ID.
