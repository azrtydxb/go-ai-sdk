...
- Root `go.mod` stays zero-dependency; new external deps go to
  `contrib/*` nested modules only.
- Providers never retry (the ai core owns retries); ctx is threaded
  first-argument everywhere.
- Public API is Vercel-AI-SDK-parity surface — additive changes only
  outside a sanctioned breaking rev.
