...
Repo-specific: the root module is zero-dependency by policy (only
`contrib/otel` carries external deps), so osv-scanner findings on the root
`go.mod` mean the policy broke — treat that as blocking regardless of CVSS.
Trust boundaries to watch: provider wire decoding, media/result-URL
fetching (`internal/fetchmedia` SSRF guards), and MCP transports
(deliberately NOT SSRF-guarded — localhost is the dominant deployment;
